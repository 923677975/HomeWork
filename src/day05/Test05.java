...
public class Test05 {
    public static void main(String[] args) {
        String str = "销售:张三;财务:李四;销售:王五;财务:赵六;" +
                "程序:mike;程序:jerry;美工:jackson;前端:green;" +
                "前端:nick;程序:钱七;销售:alice";
        Map<String, Integer> map = new HashMap<>();
        String[] a = str.split(";");
        for (int i = 0; i < a.length; i++) {
            String[] b = a[i].split(":");
            if (map.containsKey(b[0])) {
                map.put(b[0], map.get(b[0]) + 1);
            } else {
                map.put(b[0], 1);
            }
        }
        ...
        for (Map.Entry<String, Integer> en : set) {
            String x = en.getKey();
            Integer y = en.getValue();
            System.out.println("职位：" + x + "\t员工人数：" + y);
        }
    }
}
